...
export function ExampleUsageWithPassthrough() {
  ...
  const handleApply = () => {
    const diag = applyCustomCSSWithPassthrough(currentPreset.id, cssText);
    setDiagnostics(diag);
  };
  ...
  return (
    <div>
      ...
      <div>
        {diagnostics.map((d, i) => (
          <div key={i} className={`diagnostic-${d.type}`}>
            <code>{d.property}</code>: {d.message}
          </div>
        ))}
      </div>
...
declare function generateGlowCSS(settings: any): { properties: Record<string, string> };
